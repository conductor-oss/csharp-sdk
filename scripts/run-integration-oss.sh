...
# it, mirroring the `integration_tests_oss` job in
# .github/workflows/pull_request.yml. Orkes-only tests are gated out via the
...
dotnet test Tests/conductor-csharp.test.csproj \
  ...
  --filter "Category=Integration&ServerType!=Orkes" \
